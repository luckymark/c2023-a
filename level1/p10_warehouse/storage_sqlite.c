...
const char SQL_CREATE_DB[] = "CREATE TABLE IF NOT EXISTS goods(type TEXT UNIQUE NOT NULL, name TEXT, price REAL CHECK (price>=0) DEFAULT 0, amount INTEGER CHECK (amount>=0) DEFAULT 0, note TEXT DEFAULT \"\");";
...
const char SQL_SELECT_GOODS[] = "SELECT type, name, price, amount, CASE WHEN note IS NULL THEN \"(null)\" ELSE note END AS note  FROM goods WHERE type=? OR name=? ORDER BY type ASC;";
...
bool sql_insert(Goods** pGoods, int n){
    ...
    for(int i=0; i<n; i++) {
        ...
        if(ret!=SQLITE_DONE){
            fprintf(stderr, "ERROR: cannot insert goods(type='%s', name='%s', price=%.2f, amount=%d) "
...
bool sql_update(Goods** pGoods, int n){
    ...
    for(int i=0; i<n; i++){
        ...
        if(ret!=SQLITE_DONE){
            fprintf(stderr, "ERROR: cannot update goods(type='%s', name='%s', price=%.2f, amount=%d) "
                            "with error message '%s'\n", pGoods[i]->type, pGoods[i]->name, pGoods[i]->price,
                    pGoods[i]->amount, sqlite3_errmsg(pdb));
            flag = false;
        }
...
Goods** sql_select(char* type, char* name){
    ...
    if(sqlite3_step(STMT_PRESELECT_GOODS)!=SQLITE_ROW){
        fprintf(stderr, "ERROR: cannot select goods(type='%s', name='%s') "
...
bool sql_delete(char* type, char* name){
    ...
    if(sqlite3_step(STMT_DELETE_GOODS)!=SQLITE_DONE){
        fprintf(stderr, "ERROR: cannot delete goods(type='%s', name='%s') "
                        "with error message '%s'\n", type, name, sqlite3_errmsg(pdb));
        return false;
    }else return true;
};
